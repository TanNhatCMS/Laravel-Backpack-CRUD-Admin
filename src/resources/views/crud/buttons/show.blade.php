@if ($crud->hasAccess('show', $entry))
	...
	<a href="{{ url($crud->route.'/'.$entry->getKey().'/show') }}" bp-button="show" class="btn btn-sm btn-link">
		<i class="la la-eye"></i> <span>{{ trans('tannhatcms::crud.preview') }}</span>
	</a>
	...
	<div class="btn-group">
	  <a href="{{ url($crud->route.'/'.$entry->getKey().'/show') }}" class="btn btn-sm btn-link pr-0">
	  	<span><i class="la la-eye"></i> {{ trans('tannhatcms::crud.preview') }}</span>
	  </a>
	  ...
	  <ul class="dropdown-menu dropdown-menu-right">
  	    <li class="dropdown-header">{{ trans('tannhatcms::crud.preview') }}:</li>
