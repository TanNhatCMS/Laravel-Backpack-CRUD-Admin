@if ($crud->hasAccess('update', $entry))
	...
	<a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" bp-button="update" class="btn btn-sm btn-link">
		<i class="la la-edit"></i> <span>{{ trans('tannhatcms::crud.edit') }}</span>
	</a>
	...
	<div class="btn-group">
	  <a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" class="btn btn-sm btn-link pr-0">
	    <span><i class="la la-edit"></i> {{ trans('tannhatcms::crud.edit') }}</span>
	  </a>
	  ...
	  <ul class="dropdown-menu dropdown-menu-right">
  	    <li class="dropdown-header">{{ trans('tannhatcms::crud.edit_translations') }}:</li>
